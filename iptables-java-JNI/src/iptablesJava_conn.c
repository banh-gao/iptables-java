...
#include "net_sf_iptablesJava_connection_NetFilterConnTask.h"
...
JNIEXPORT void JNICALL Java_net_sf_iptablesJava_connection_NetFilterConnTask_init (JNIEnv * javaEnv, jobject javaObj) {
	env = javaEnv;
	obj = javaObj;

	jclass cls = (*env)->GetObjectClass(env, obj);
	buildMethod = (*env)->GetMethodID(env, cls, "getConnection", "(Ljava/lang/String;)Lnet/sf/iptablesJava/connection/Connection;");
...
JNIEXPORT void JNICALL Java_net_sf_iptablesJava_connection_NetFilterConnTask_deinit (JNIEnv * javaEnv, jobject javaObj) {
	u_int8_t family = AF_INET;
	nfct_query(h, NFCT_Q_DUMP, &family);
}
